...
namespace NuGet.DependencyResolver
{
    public class WalkContext
    {
            ...
            NuGetFramework framework)
        {
            ...
            ForEach(root, node =>
            {
                node.Item = Resolve(resolvedItems, resolvers, node.Key, framework);
                if (node.Item == null)
                {
        ...
        public IList<ResolveResult> GetResolvedItems()
        {
            var resolved = new List<ResolveResult>();
            foreach (var item in _usedItems.Values)
            {
                var result = new ResolveResult
                {
                    DependencyProvider = item.Resolver,
                    LibraryDescription = new LibraryDescription
                    {
                        LibraryRange = item.Description.LibraryRange,
                        Identity = item.Key,
                        Path = item.Description.Path,
                        Dependencies = item.Dependencies.SelectMany(CorrectDependencyVersion).ToList(),
                        Resolved = item.Description.Resolved
                    }
                };
